...
NemoCUA does not currently support host-local inference. Do not set `NEMOCLAW_PROVIDER=ollama` when onboarding a `nemocua` sandbox; that combination exits with `Unsupported host-local inference application 'nemocua'`. Choose a non-host-local inference provider instead.
</Warning>
